...
This prompt is color-scheme-able.  You can invoke it thus:
  ...
  prompt tema [ 8bit ] [<color1> [<color2> [<color3>] [<color4>]]
...
prompt_tema_setup () {
  ...
  # Colour scheme
  prompt_tema_color1=${1:-'cyan'}    # hyphens
  prompt_tema_color2=${2:-'green'}   # current directory
  prompt_tema_color3=${3:-'cyan'}    # user@host
  prompt_tema_color4=${4:-'white'}   # user input

  local prompt_gfx_bbox 
  prompt_gfx_tbox="%B%F{$prompt_tema_color1}${prompt_gfx_tlc}%b%F{$prompt_tema_color1}${prompt_gfx_hyphen}"
  prompt_gfx_bbox="%B%F{$prompt_tema_color1}${prompt_gfx_blc}${prompt_gfx_hyphen}%b%F{$prompt_tema_color1}"

  # This is a cute hack.  Well I like it, anyway.
  prompt_gfx_bbox_to_mbox=$'%{\e[A\r'"%}%B%F{$prompt_tema_color1}${prompt_gfx_mlc}%b%F{$prompt_tema_color1}${prompt_gfx_hyphen}%{"$'\e[B%}'

  prompt_l_paren="%B%F{black}("
  prompt_r_paren="%B%F{black})"

  prompt_user_host="%b%F{$prompt_tema_color3}%n%B%F{$prompt_tema_color3}@%b%F{$prompt_tema_color3}%m"

  prompt_line_1a="$prompt_gfx_tbox$prompt_l_paren%B%F{$prompt_tema_color2}%~$prompt_r_paren%b%F{$prompt_tema_color1}"
  prompt_line_1b="$prompt_l_paren$prompt_user_host$prompt_r_paren%b%F{$prompt_tema_color1}${prompt_gfx_hyphen}"
  ...
  add-zsh-hook precmd prompt_tema_precmd
}
...
prompt_tema_precmd() {
  ...
  zle_highlight[(r)default:*]="default:fg=$prompt_tema_color4,bold"
}
...
prompt_tema_choose_prompt () {
  ...
  prompt_line_1="$prompt_gfx_tbox$prompt_l_paren%B%F{$prompt_tema_color2}%$prompt_pwd_size<...<%~%<<$prompt_r_paren%b%F{$prompt_tema_color1}$prompt_gfx_hyphen"
}
